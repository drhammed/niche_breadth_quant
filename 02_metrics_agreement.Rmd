...
This markdown analyzes metrics agreement among 11 niche breadth metrics across 12 simulation scenarios and empirical data. Here, I examine:
...
if (file.exists(empirical_file)) {
  ...
  print(sprintf("Loaded %d scenario results + empirical data", length(all_results)))
} else {
  print(sprintf("Loaded %d scenario results (no empirical data found)", length(all_results)))
}
```
...
aggregate_niche_breadth <- function(df) {
  ...
  df %>%
    ...
    dplyr::summarise(dplyr::across(dplyr::all_of(metric_cols), ~mean(.x, na.rm = TRUE)))
}
...
ggplot(mean_corr_long, aes(x = metric, y = scenario, fill = correlation)) +
  ...
  labs(title = "Mean Pearson Correlation with Oracle Niche Breadth",
...
knitr::kable(round(mean_corr_df[, -ncol(mean_corr_df)], 3),
             caption = "Mean Pearson Correlations with Oracle Niche Breadth")
```
...
```{r variability-plot}
# Coefficient of variation (CV) of mean values across scenarios
# CV = SD/|mean| — measures relative variability, comparable across different scales
metric_means <- apply(mean_df, 2, mean, na.rm = TRUE)
metric_sds <- apply(mean_df, 2, sd, na.rm = TRUE)
metric_cv <- metric_sds / abs(metric_means)
metric_var_df <- data.frame(Metric = names(metric_cv), CV = metric_cv)

ggplot(metric_var_df, aes(x = reorder(Metric, -CV), y = CV)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = "Metric Variability Across Scenarios",
       subtitle = "Coefficient of variation of mean values",
       x = "Metric", y = "CV across scenarios") +
  theme_minimal()
```
...
# Calculate Pearson correlation matrix for each scenario
# For simulation scenarios: compute correlations per iteration, then average
corr_list <- lapply(names(matrices_list), function(name) {
  raw_df <- all_results[[name]]
  if (!is.null(raw_df) && "iteration" %in% colnames(raw_df)) {
    # Simulation: correlate within each iteration, then average
    iterations <- unique(raw_df$iteration)
    iter_corrs <- lapply(iterations, function(iter) {
      iter_df <- raw_df[raw_df$iteration == iter, metrics, drop = FALSE]
      cor(iter_df, method = "pearson", use = "pairwise.complete.obs")
    })
    Reduce("+", iter_corrs) / length(iter_corrs)
  } else {
    # Empirical: no iterations, correlate directly
    df <- matrices_list[[name]]
    cor(df[, metrics], method = "pearson", use = "pairwise.complete.obs")
  }
})
names(corr_list) <- names(matrices_list)
...
## Empirical vs Simulated: Distance-Based Comparison

This section compares the empirical correlation structure to all simulation scenarios using Euclidean distance between flattened pairwise correlation vectors.

```{r distance-heatmap, eval=exists("result_df_empirical"), fig.width=12, fig.height=10}

# --- (A) Euclidean distance heatmap between all scenario correlation vectors ---

# corr_flat_list already contains flattened lower-triangle correlation vectors
# (from the Correlation Structure per Scenario section above)

# Build distance matrix between all scenarios (including Empirical)
corr_flat_mat <- do.call(rbind, corr_flat_list)
rownames(corr_flat_mat) <- names(corr_list)

dist_raw <- as.matrix(dist(corr_flat_mat, method = "euclidean"))

# Normalize to [0, 1] range
dist_between <- dist_raw / max(dist_raw)

# Pretty labels for scenarios
pretty_label <- function(name) {
  name_clean <- gsub("result_df_", "", name)
  name_clean <- gsub("_", " - ", name_clean)
  # Capitalize first letter of each word
  name_clean <- gsub("(^|\\s|-)([a-z])", "\\1\\U\\2", name_clean, perl = TRUE)
  name_clean
}

pretty_names <- sapply(rownames(dist_between), pretty_label)
rownames(dist_between) <- pretty_names
colnames(dist_between) <- pretty_names

# Heatmap of distances
dist_long <- as.data.frame(as.table(dist_between))
colnames(dist_long) <- c("Scenario1", "Scenario2", "Distance")

fig_dist_heatmap <- ggplot(dist_long, aes(x = Scenario1, y = Scenario2, fill = Distance)) +
  geom_tile(color = "white", linewidth = 0.3) +
  scale_fill_gradient2(
    low = "#2166AC", mid = "white", high = "#B2182B",
    midpoint = median(dist_long$Distance),
    name = "Distance"
  ) +
  labs(title = "(A)") +
  theme_publication() +
...
```{r distance-density, eval=exists("result_df_empirical"), fig.width=8, fig.height=6}
# --- (B) Density plot: Empirical vs Simulation distances vs Simulation vs Simulation ---

# Identify empirical row
emp_label <- pretty_label("Empirical")
sim_labels <- setdiff(rownames(dist_between), emp_label)

# Empirical vs simulation distances
emp_vs_sim <- dist_between[emp_label, sim_labels]

# Simulation vs simulation distances (upper triangle, excluding empirical)
sim_dist <- dist_between[sim_labels, sim_labels]
sim_vs_sim <- sim_dist[upper.tri(sim_dist)]

density_df <- rbind(
  data.frame(Distance = emp_vs_sim, Type = "Empirical vs Simulation"),
  data.frame(Distance = sim_vs_sim, Type = "Simulation vs Simulation")
)

fig_dist_density <- ggplot(density_df, aes(x = Distance, fill = Type)) +
  geom_density(alpha = 0.5, color = "black") +
  scale_fill_manual(values = c("Empirical vs Simulation" = "#E41A1C",
                                "Simulation vs Simulation" = "gray70")) +
  labs(title = "(B)",
...
```{r distance-percentile, eval=exists("result_df_empirical"), fig.width=8, fig.height=6}
# --- (C) Percentile bar plot: where does each scenario's distance to empirical fall? ---

# For each simulation scenario, compute percentile of its distance to empirical
# among all simulation-simulation distances
percentile_df <- data.frame(
  Scenario = sim_labels,
  Distance_to_Empirical = emp_vs_sim[sim_labels],
  Percentile = sapply(emp_vs_sim[sim_labels], function(d) mean(sim_vs_sim <= d)),
  stringsAsFactors = FALSE
)

# Add response type for coloring
percentile_df$Response_Type <- ifelse(grepl("Asy|asy", percentile_df$Scenario, ignore.case = TRUE),
                                       "Asymmetric", "Symmetric")

# Sort by percentile
percentile_df <- percentile_df[order(percentile_df$Percentile), ]
percentile_df$Scenario <- factor(percentile_df$Scenario, levels = percentile_df$Scenario)

fig_dist_percentile <- ggplot(percentile_df, aes(x = Percentile, y = Scenario, fill = Response_Type)) +
  geom_bar(stat = "identity", width = 0.7) +
  scale_fill_manual(values = c("Asymmetric" = "#E41A1C", "Symmetric" = "#377EB8"),
                    name = "Response type") +
  labs(title = "(C)",
       x = "Percentile", y = "Simulation Scenario") +
  theme_publication() +
  theme(axis.text.y = element_text(size = 8))
```

```{r distance-combined, eval=exists("result_df_empirical"), fig.width=16, fig.height=12}
# --- Combined figure ---
fig_combined_distance <- ggarrange(
  fig_dist_heatmap,
  ggarrange(fig_dist_density, fig_dist_percentile, ncol = 2, widths = c(1, 1)),
  nrow = 2, heights = c(1.2, 1)
)

print(fig_combined_distance)

ggsave(file.path(figures_dir, "fig_empirical_vs_simulated_distances.png"),
       fig_combined_distance, width = 16, height = 12, dpi = 300)
ggsave(file.path(figures_dir, "fig_empirical_vs_simulated_distances.pdf"),
       fig_combined_distance, width = 16, height = 12)
```

## Distribution of Pairwise Method Correlations

```{r correlation-density, eval=exists("result_df_empirical"), fig.width=8, fig.height=6}
# Density plot of pairwise correlations between methods: Empirical vs Simulated

# Empirical correlation values (lower triangle)
emp_corr_vals <- flatten_corr(corr_list[["Empirical"]])

# Simulated correlation values (pool all scenario lower triangles)
sim_scenario_names <- setdiff(names(corr_list), "Empirical")
sim_corr_vals <- unlist(lapply(sim_scenario_names, function(nm) flatten_corr(corr_list[[nm]])))

corr_density_df <- rbind(
  data.frame(Correlation = emp_corr_vals, Type = "Empirical"),
  data.frame(Correlation = sim_corr_vals, Type = "Simulated")
)

fig_corr_density <- ggplot(corr_density_df, aes(x = Correlation, fill = Type)) +
  geom_density(alpha = 0.5, color = "black") +
  scale_fill_manual(values = c("Empirical" = "#E41A1C", "Simulated" = "gray70")) +
  labs(title = "Distribution of pairwise method correlations",
       x = "Pairwise correlation between methods",
       y = "Density") +
  theme_publication() +
  theme(legend.position = "bottom",
        legend.background = element_rect(fill = "white", color = NA))

print(fig_corr_density)

ggsave(file.path(figures_dir, "fig_pairwise_correlation_density.png"),
       fig_corr_density, width = 8, height = 6, dpi = 300)
ggsave(file.path(figures_dir, "fig_pairwise_correlation_density.pdf"),
       fig_corr_density, width = 8, height = 6)
```
...
emp_cor_mat <- cor(result_df_empirical[, -1], method = "pearson",
...
```{r summary}
# Best performing metrics (highest mean ABSOLUTE correlation with oracle)
if (nrow(mean_corr_df) > 0) {
  # Mean of absolute correlations (strength regardless of direction)
  overall_mean_abs_corr <- sapply(
    colnames(mean_corr_df)[-ncol(mean_corr_df)],
    function(m) mean(abs(mean_corr_df[[m]]), na.rm = TRUE)
  )
  best_metrics <- sort(overall_mean_abs_corr, decreasing = TRUE)

  cat("Metrics ranked by mean |ρ| with oracle (absolute correlation):\n")
...
emp_cor_mat <- cor(result_df_empirical[, emp_metrics], method = "pearson",
                   use = "pairwise.complete.obs")
...
fig_emp_heatmap <- ggplot(emp_cor_long, aes(x = Metric1, y = Metric2, fill = Correlation)) +
  ...
  labs(
    ...
    subtitle = "Hierarchically clustered Correlations among 11 niche breadth metrics",
    x = "", y = ""
  ) +
  theme_publication() +
  theme(
...
# Display
plot(hc, main = "Metric Clustering (Empirical Data)",
     sub = "Average linkage clustering based on 1 - Pearson correlation",
     xlab = "", ylab = "Distance (1 - r)")
```
