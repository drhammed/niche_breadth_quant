...
Generates community data under 12 different scenarios (combinations of response types, breadth distributions, and number of environmental variables) and calculates 11 niche breadth metrics for each.
...
3. Calculating all 11 niche breadth metrics
...
run_iteration <- function(iteration, scenario) {
  ...
  # Blonder hypervolume (simulation mode: samples.per.point=10, no scaling)
  hv_result <- hypervolume_blond_fun(sim.com, env_vars, empirical = FALSE)

  # GAM-based niche breadth (general version)
  gam_result <- estimate_nicheBreadth_Gam_general(
    sim.com, env_vars,
    summary_method = "determinant",
    model_type = "hybrid",
    joint_engine = "ti",
    verbose = FALSE
  )

  # Latent variable model (covariance-based, no env needed)
  latent_result <- estimate_nicheBreadth_Latents(sim.com, nlv = NLV, verbose = FALSE)

  # Weighted average environmental distance
  dist_result <- estimate_nicheBreadth_avgDist_weighted(sim.com, env_vars)

  # Alpha diversity contribution mean
  lpcd_mean_result <- estimate_nicheBreadth_from_alphaMean(sim.com)

  # Alpha diversity contribution SD
  ...
  result <- data.frame(
    ...
    LPCD_mean = lpcd_mean_result[match(co_occ$sci.name, names(lpcd_mean_result))],
    LPCD_sd = lpcd_sd_result[match(co_occ$sci.name, names(lpcd_sd_result))],
...
for (i in 1:nrow(SCENARIOS)) {
  ...
  scenario_results <- foreach(
    ...
    .export = c("generate_community", "generate_abundances_symmetric",
                ...
                "estimate_nicheBreadth_Gam_general", "fit_gam_general",
                "build_gam_formula", "weighted_cov", "summarize_breadth_from_cov",
                "estimate_nicheBreadth_Latents",
                "estimate_nicheBreadth_avgDist_weighted",
                "estimate_nicheBreadth_from_alphaMean",
                "estimate_nicheBreadth_from_alphaSD",
                "eliminate_cols",
...
# Correlation matrix
cor_mat <- cor(result_df_empirical[, -1], method = "pearson", use = "complete.obs")
